...
import {SATELLITE_POSITION_URL} from "../constants";
import {SAT_API_KEY} from "../constants";
...
class WorldMap extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.satData !== this.props.satData) {
            ...
            // step1: prepare urls for each selected satellite
            ...
            // step2: pars sats position
            // axios.all(urls)
            //     .then(
            //         axios.spread((...args) => {
            //             console.log("look at me ->", args)
            ...
            //     })
            //     .catch(err => {
            //         console.log("there is an err", err.message);
            //     })
    ...
    track = data =>{
        if (!data || !data[0].hasOwnProperty('positions')) {
            throw new Error('no posiiton data');
            return;
        }
    ...
    drawSat = (sat, pos) => {
        ...
        console.log("ok")
        context2.fillStyle = this.color(satname);
        context2.beginPath();
